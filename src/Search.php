                   ...
                   // Due date + progress
                    if (in_array($orig_id, [151, 158, 181, 186])) {
                       ...
                       // No due date in waiting status
                        ...
                        // Alteração
                        $current_date_time_to_resolve = $item->fields['current_date_time_to_resolve'];
                        
                        /*
                        var_dump($teste);
                        echo "<hr>";
                        /*
                        var_dump($timestampTeste);
                        echo "<hr>";
                        */
                        $percentage  = 0;
                        $totaltime   = 0;
                       ...
                       // define correct sla field
                        ...
                        if ($item->isField($slaField) && $item->fields[$slaField] != 0) { // Have SLA
                            ...
                            // Alteração
                            if($current_date_time_to_resolve !== null){
                                $currenttime = $sla->getActiveTimeBetween(
                                    $current_date_time_to_resolve,
                                    date('Y-m-d H:i:s')
                                );
                            }else{
                                $currenttime = $sla->getActiveTimeBetween(
                        ...
                        if ($totaltime != 0) {
                            ...
                            /* Inicio do Debug
                            echo "<pre style='background:#333!important; color:#fff;'>";
                            var_dump($data["id"]);
                            echo "currentTime => " . $item->fields['date'] . " - " . date('Y-m-d H:i:s');
                            echo "<br>";
                            echo "waitingtim => " . $waitingtime;
                            echo "<br>";
                            echo "totaltime => " . $item->fields['date'] . " - " . $data[$ID][0]['name'];
                            echo "<br>";
                            echo "(100 * (currenttime - waitingtime) / (totaltime - waitingtime))";
                            echo "</br>";
                            echo "(100 * (" .$currenttime. " - " .$waitingtime. ") / (" .$totaltime. " - " .$waitingtime. ")";
                            echo "</br>";
                            echo " = " . $percentage ;
